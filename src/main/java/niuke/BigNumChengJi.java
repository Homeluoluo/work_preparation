...
/**
 * @Classname BigNumChengJi
 * @Description TODO
 * @Date 2019/4/6 14:34
 * @Created by luojia
 */
public class BigNumChengJi {
...
//        用b的每一位依次去乘a
        for (int i = n-1; i >=0; i--) {
            for (int j = m-1; j >=0 ; j--) {
                ...
                toHigh=num/10;
                num=num%10;
                result[n-1-i][m-1-j+n-1-i]=(char)(num+'0');
            }
            if(toHigh>0){
                result[n-1-i][m+n-1-i]=(char)(toHigh+'0');
                toHigh=0;
            }
        }
        //相乘的结果相加
        ...
        for (; i <m+n ; i++) {
            int num=toHigh;
            for (int j = 0; j < n; j++) {
                num+=Integer.parseInt(result[j][i]+"");
            }
            result[n][i]=(char)(num%10+'0');
            toHigh=num/10;
        }
        if(toHigh>0){
            result[n][i+1]=(char)(toHigh+'0');
        }
